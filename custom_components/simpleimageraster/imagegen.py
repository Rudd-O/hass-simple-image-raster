    ...
    for element in payload:
        ...
        if element["type"] == "rectangle":
            check_for_missing_required_arguments(
                element, ["x_start", "x_end", "y_start", "y_end"], "rectangle"
            )
            img_rect = ImageDraw.Draw(img)
            rect_fill = getIndexColor(element["fill"]) if "fill" in element else None
            rect_outline = (
                getIndexColor(element["outline"]) if "outline" in element else "black"
            )
                ...
                for y in range(element["y_repeat"]):
                    img_rect_pattern.rounded_rectangle(
                        [
                            (
                                element["x_start"]
                                + x * (element["x_offset"] + element["x_size"]),
                                element["y_start"]
                                + y * (element["y_offset"] + element["y_size"]),
                            ),
                            (
                                element["x_start"]
                                + x * (element["x_offset"] + element["x_size"])
        ...
        if element["type"] == "ellipse":
            check_for_missing_required_arguments(
                element, ["x_start", "x_end", "y_start", "y_end"], "ellipse"
            )
        ...
        if element["type"] == "text":
            ...
            if "max_width" in element:
                text = get_wrapped_text(
                    str(element["value"]), font, line_length=element["max_width"]
                )
                anchor = None
            else:
        ...
        if element["type"] == "multiline":
            ...
            _LOGGER.debug(
                "Got Multiline string: %s with delimiter: %s"
            ...
            if isinstance(element["value"], str):
                ...
                lst = [y for x in element["value"] for y in x.splitlines()]
        ...
        if element["type"] == "icon":
            ...
            fill = (
                ...
                if "color" in element
                else getIndexColor(element["fill"])
                if "fill" in element
                else "black"
            )
            d.text(
        ...
        if element["type"] == "dlimg":
            check_for_missing_required_arguments(
                element, ["x", "y", "url", "xsize", "ysize"], "dlimg"
            )
            url = element["url"]
            pos_x = element["x"]
            pos_y = element["y"]
        ...
        if element["type"] == "diagram":
            ...
            if "bars" in element:
                ...
                bar_width = math.floor(
                    (width - offset_lines - ((barcount + 1) * bar_margin)) / barcount
                )
                ...
                for bar in bars:
                    ...
                    d.text(
                        ...
                        str(name),
                        fill=getIndexColor(legend_color),
                        font=font,
                        anchor="mm",
                    )
                    img_draw.rectangle(
        ...
        if element["type"] == "plot":
            ...
            for plot in element["data"]:
                ...
                states = [
                    (datetime.fromisoformat(s["last_changed"]), float(s["state"]))
                    for s in states
                    if is_decimal(s["state"])
                ]
            ...
            if element.get("debug", False):
                img_draw.rectangle(
                    ...
                    fill=None,
                    outline=getIndexColor("black"),
                    width=1,
                )
                img_draw.rectangle(
                    [
                        (diag_x, diag_y),
                        (diag_x + diag_width - 1, diag_y + diag_height - 1),
                    ],
                    fill=None,
                    outline=getIndexColor("red"),
                    width=1,
                )
            ...
            if yaxis is not None:
                if yaxis_grid is not None:
                    ...
                    while curr <= max_v:
                        curr_y = round(
                            diag_y + (1 - ((curr - min_v) / spread)) * (diag_height - 1)
                        )
                        grid_points.extend(
                            (x, curr_y)
            ...
            for plot, data in zip(element["data"], raw_data):
                ...
                img_draw.line(
                    xy,
                    fill=getIndexColor(plot.get("color", "black")),
            ...
            if ylegend_pos == "left":
                img_draw.text(
                    (x_start, y_start),
                    str(max_v),
                    fill=getIndexColor(ylegend_color),
                    font=ylegend_font,
                    anchor="lt",
                )
                img_draw.text(
            ...
            if yaxis is not None:
                img_draw.rectangle(
                    [
                        (diag_x, diag_y),
                        (diag_x + yaxis_width - 1, diag_y + diag_height - 1),
                    ],
                    width=0,
                    fill=getIndexColor(yaxis_color),
                )
        ...
        if element["type"] == "progress_bar":
            ...
            if direction == "right":
                img_draw.rectangle(
                    [(x_start, y_start), (x_start + progress_width, y_end)], fill=fill
                )
            elif direction == "left":
                img_draw.rectangle(
                    [(x_end - progress_width, y_start), (x_end, y_end)], fill=fill
                )
            elif direction == "up":
                img_draw.rectangle(
                    [(x_start, y_end - progress_height), (x_end, y_end)], fill=fill
                )
            elif direction == "down":
                img_draw.rectangle(
                    [(x_start, y_start), (x_end, y_start + progress_height)], fill=fill
                )
